...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	createListItemsQuery := fmt.Sprintf("INSERT INTO %s (list_id, item_id) values($1, $2)", "lists_items")
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id
		...
		todoItemsTable, "lists_items", userListsTable)
...
func (r *TodoItemPostgres) GetById(userId, listId, itemId int) (todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT * FROM %s ti INNER JOIN %s li on li.item_id = ti.id
						INNER JOIN %s ul on ul.list_id = li.list_id WHERE ti.id = $1 AND li.list_id = $2 AND ul.user_id = $3`,
		todoItemsTable, listItemsTable, userListsTable)

	err := r.db.Get(&item, query, itemId, listId, userId)
	if err != nil {
		return item, err
	}
